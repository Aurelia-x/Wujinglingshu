...
def motion_match_or_not(keymotion_path:str,input_file:dict) -> bool:
        #score = calculate_skeleton_similarity(load_skeleton(input_file),load_skeleton(keymotion_path))
        if isinstance(input_file,dict):
            score = calculate_skeleton_similarity(input_file,load_skeleton(keymotion_path))
            if score<0.25:
                return True
            else:
                print(f"\033[31m{score}\033[0m")
                return False
        else:
