...
def generate_waist_node(input_file, output_file):
    """
    读取输入JSON文件中的左肩、右肩、左髋、右髋坐标，
    以0.66, 0.66, 0.33, 0.33的权重计算新的腰部节点，
    并将结果保存到输出文件中
    ...
    # 提取所需节点的坐标
    nodes = ['left_shoulder', 'right_shoulder', 'left_hip', 'right_hip']
    weights = [0.66, 0.66, 0.33, 0.33]
    
    # 计算权重总和（用于归一化）
    weight_sum = sum(weights)
    
    # 计算腰部节点的x, y, z坐标
    waist_x = sum(data[node]['x'] * weight for node, weight in zip(nodes, weights)) / weight_sum
    waist_y = sum(data[node]['y'] * weight for node, weight in zip(nodes, weights)) / weight_sum
    waist_z = sum(data[node]['z'] * weight for node, weight in zip(nodes, weights)) / weight_sum
    
    # 计算可见度（取四个节点可见度的平均值）
    waist_visibility = sum(data[node]['visibility'] for node in nodes) / len(nodes)
            ...
            if frame_count % interval == 0:
                ...
                if success and landmarks:
                    ...
                    if skeleton_data:
                        ...
                        # 添加标签（包括时间信息，腰部节点）
                        generate_waist_node(json_path,json_path)
